...
public class CurrentWaveDisplay : MonoBehaviour {
    ...
    void Update()
    {
        ...
        else
        {
            ...
            for (int i = 0; i < transform.childCount; i++)
            {
                GameObject slime = transform.GetChild(i).gameObject;
                int remaining = game.m_spawnsLeft[i];
                slime.GetComponentInChildren<UnityEngine.UI.Text>().text = "x" + remaining.ToString();
            }
        }
    }
}
